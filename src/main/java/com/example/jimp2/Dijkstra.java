...
public class Dijkstra {
    ...
    public boolean doDijkstra(int start, int finish)  {
        ...
        if(last != rowNum*colNum){
            ...
            st.wypisz("Nie udało się znaleźć drogi :(");
    ...
    public void showPathDij(int from, int to) {
        if(!path.containsKey(to) || to < from){
            ...
            st.wypisz("Nie da się odnaleźć takiej drogi");
            return;
        }
//        while (from != to) {
...
//        }
    try {
        ...
        System.out.println("Koszt przebycia całej ścieżki: "+suma);
        st.wypisz("Koszt przebycia całej ścieżki: "+suma);
    }catch (NullPointerException e){
        System.out.println("[dijkstra]: Nie ma takiej drogi");
        st.wypisz("[dijkstra]: Nie ma takiej drogi");
    }
